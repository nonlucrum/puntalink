...
exports.addManualMuro = addManualMuro;
exports.updateManualMuro = updateManualMuro;
exports.deleteManualMuro = deleteManualMuro;
exports.reorderProjectMuros = reorderProjectMuros;
const db_1 = __importDefault(require("./config/db"));
const Muro_1 = require("../models/Muro");
/**
 * Agregar un muro manual en una posición específica (transaccional)
 */
async function addManualMuro(pk_proyecto, muroData, position) {
    ...
    try {
        ...
        if (pos === 'end') {
            const maxResult = await client.query('SELECT COALESCE(MAX(num), 0) as max_num FROM muro WHERE pk_proyecto = $1', [pk_proyecto]);
            num = parseInt(maxResult.rows[0].max_num) + 1;
        }
        else if (pos === 'start') {
            // Desplazar todos los muros +1 para hacer espacio en posición 1
            await client.query('UPDATE muro SET num = num + 1 WHERE pk_proyecto = $1', [pk_proyecto]);
            num = 1;
        }
        else if (pos === 'after' && position.reference_pid) {
            const refResult = await client.query('SELECT num FROM muro WHERE pid = $1 AND pk_proyecto = $2', [position.reference_pid, pk_proyecto]);
            ...
            await client.query('UPDATE muro SET num = num + 1 WHERE pk_proyecto = $1 AND num > $2', [pk_proyecto, refNum]);
            num = refNum + 1;
        }
 ...
 */
async function updateManualMuro(pid, pk_proyecto, muroData) {
    const client = await db_1.default.connect();
    try {
        await client.query('BEGIN');
        // Verificar que el muro existe y es manual
        const check = await client.query('SELECT pid, origen FROM muro WHERE pid = $1 AND pk_proyecto = $2', [pid, pk_proyecto]);
 ...
 */
async function deleteManualMuro(pid, pk_proyecto) {
    const client = await db_1.default.connect();
    try {
        await client.query('BEGIN');
        const result = await client.query('DELETE FROM muro WHERE pid = $1 AND pk_proyecto = $2 RETURNING pid', [pid, pk_proyecto]);
        if (result.rowCount === 0) {
            throw new Error('Muro no encontrado');
        }
        // Compactar numeración después de eliminar
        await (0, Muro_1.compactRenumber)(pk_proyecto, client);
        await client.query('COMMIT');
        return true;
    }
    catch (error) {
 ...
 */
async function reorderProjectMuros(pk_proyecto, ordering) {
    const client = await db_1.default.connect();
    try {
        await client.query('BEGIN');
        // Usar números temporales negativos para evitar conflictos de unicidad
        for (let i = 0; i < ordering.length; i++) {
            await client.query('UPDATE muro SET num = $1 WHERE pid = $2 AND pk_proyecto = $3', [-(i + 1), ordering[i].pid, pk_proyecto]);
        }
        // Ahora asignar los números definitivos
        for (const item of ordering) {
            await client.query('UPDATE muro SET num = $1 WHERE pid = $2 AND pk_proyecto = $3', [item.num, item.pid, pk_proyecto]);
        }
        await client.query('COMMIT');
    }
    catch (error) {
