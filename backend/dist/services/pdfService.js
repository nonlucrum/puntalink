...
function generarInformePDF(reportData) {
    console.log('[pdfService] Iniciando generación PDF (nuevo formato)...');
    const { paneles: rawPaneles, projectInfo, tablaMuertos, filasArmadoPairs, totals, user, reportImage, } = reportData;
    const panelesConvertidos = rawPaneles.map(p => p);
...
function generarInformePaneles(paneles, projectInfo, tablaMuertos, tablaArmado, user, reporteMacizos, reportImage) {
...
function crearPortada(doc, projectInfo, reportImage) {
    ...
    const lineH = 28;
    const drawCoverField = (labelEn, labelEs, value) => {
...
function crearPaginaProyecto(doc, projectInfo, user) {
    ...
    if (fs_1.default.existsSync(imgPath)) {
        try {
            ...
            console.warn('[pdfService] No se pudo insertar imagenInfo.png:', err?.message || err);
        }
    }
}
// ==================== CALCULATIONS PAGE ====================
...
function crearTablaArmadoPorMuertoPairs(doc, pairs, totals) {
    const startX = 40;
    const startY = 70;
    const rowH = 18;
    ...
    const drawTitle = (y) => {
        doc.fontSize(11).fillColor(AZUL).font('Helvetica-Bold')
            .text('Tabla 1. Muerto corrido para muro TILT-UP. Long.: Longitud o longitudinal, Trans.: Transversal, Direc.: Dirección', startX, y - 50, { width: totalTableW });
        doc.font('Helvetica');
    };
    // === Group header ===
    ...
    pairs.forEach((pair, idx) => {
        ...
        const dmCells = [
            ...
            { text: pair.deadman.muros, w: cols[2].w, align: 'left' },
        ...
        dmCells.forEach((c) => {
            ...
            doc.fillColor('#000000').fontSize(8)
                .text(c.text, x + 2, y + (pairH - 8) / 2, { width: c.w - 4, align: (c.align || 'center') });
            x += c.w;
        });
...
function crearTablaEspaciado(doc, pairs) {
    const startX = 40;
    const startY = 70;
    const rowH = 20;
    ...
    pairs.forEach((pair, idx) => {
        ...
        rowData.forEach((c) => {
            ...
            doc.fillColor('#000000').fontSize(8)
                .text(c.text, x + 3, y + 5, { width: c.w - 6, align: c.align });
            x += c.w;
        });
        y += rowH;
    });
    ...
    doc.fontSize(8).fillColor('#555')
        .text('Revisar que la distancia X (distancia horizontal) que va desde la cara del muro al punto de apuntalamiento del brace (centro del muerto) coincida con los shop tickets de la ingeniería de izaje.', startX, y, { width: 450, align: 'justify' });
}
// ==================== LEGACY TABLE (single-row format) ====================
function crearTablaArmadoLegacy(doc, filas) {
    const startX = 40;
    const startY = 70;
    const rowH = 22;
    ...
    if (scl < 1)
        cols = cols.map(c => ({ ...c, w: Math.max(22, Math.floor(c.w * scl)) }));
    const groupWidth = (i, len) => cols.slice(i, i + len).reduce((a, c) => a + c.w, 0);
    const drawHeaders = (y) => {
        const h1 = 24;
        const h2 = 18;
        let x = startX;
        const drawGrp = (lbl, from, len) => {
            const w = groupWidth(from, len);
            doc.save();
            doc.rect(x, y, w, h1).fillAndStroke('#E8F4FD', AZUL);
        ...
        drawGrp('DEADMAN', 0, 6);
        drawGrp('ACERO', 6, 4);
        drawGrp('CONCRETO', 10, 2);
        drawGrp('ALAMBRE', 12, 2);
        const labels = ['#', 'EJE', 'MUROS', 'LARGO', 'ALTO', 'ANCHO', '#', 'LONGITUD', 'PESO', 'DIRECCIÓN', 'VOL', 'PESO', 'LONGITUD', 'PESO'];
        x = startX;
        for (let i = 0; i < cols.length; i++) {
            doc.save();
            doc.rect(x, y + h1, cols[i].w, h2).fillAndStroke('#F6F9FC', '#CCCCCC');
    ...
    filas.forEach((r, idx) => {
        ...
        cells.forEach((text, i) => {
            ...
            doc.fillColor('#000').fontSize(9).text(String(text), x + 3, y + 6, { width: cols[i].w - 6, align: 'center' });
            x += cols[i].w;
        });
        y += rowH;
    });
