...
const calculoVientoMuros = async (req, res) => {
    try {
        ...
        for (const resultado of resultados) {
            if (resultado.pid) {
                try {
                    await (0, Muro_1.updateMuroWindCalculations)(resultado.pid, resultado.qz_kPa, resultado.presion_kPa, resultado.fuerza_kN);
 ...
 */
const actualizarCamposEditables = async (req, res) => {
    try {
        ...
        const muroActualizado = await (0, Muro_1.updateMuroEditableFields)(parseInt(pid), angulo_brace, npt, x_braces, tipo_construccion, tipo_brace_seleccionado, eje);
        if (!muroActualizado) {
...
exports.actualizarCamposEditables = actualizarCamposEditables;
/**
...
const calcularBraces = async (req, res) => {
    try {
        ...
        const resultMuro = await db_1.default.query('SELECT * FROM muro WHERE pid = $1', [parseInt(pid)]);
        ...
        const muroActualizado = await (0, Muro_1.updateMuroBraceCalculations)(parseInt(pid), resultado.fbx, resultado.fby, resultado.fb, resultado.cant_b14, resultado.cant_b12, resultado.cant_b04, resultado.cant_b15, resultado.x_inserto, resultado.y_inserto);
...
exports.calcularBraces = calcularBraces;
/**
 ...
 */
const calcularBracesTiempoReal = async (req, res) => {
    try {
        const { pid } = req.params;
        ...
        const resultado = (0, calculosService_1.calculateBraceForces)(fuerza_viento_kN, // De BD
        presion_kPa, // De BD
...
const aplicarValoresGlobales = async (req, res) => {
    try {
        ...
        const muros = await (0, Muro_1.getMurosByProject)(parseInt(pk_proyecto));
        if (muros.length === 0) {
...
exports.aplicarValoresGlobales = aplicarValoresGlobales;
/**
 ...
 */
const recalcularTiposBracesMasivo = async (req, res) => {
    try {
        ...
        const resultMuros = await db_1.default.query(`
        ...
        if (actualizaciones.length > 0) {
            ...
            try {
                for (const update of actualizaciones) {
                    await db_1.default.query('UPDATE muro SET tipo_brace_seleccionado = $1 WHERE pid = $2', [update.tipo_nuevo, update.pid]);
                }
...
exports.recalcularTiposBracesMasivo = recalcularTiposBracesMasivo;
// Función auxiliar simplificada para determinación de tipo
function determinarTipoBraceSimple(alto_m, npt_m, angulo_grados, factor_w2 = 0.6) {
 ...
 */
const autoRecalcularTiposBraces = async (req, res) => {
    try {
    ...
    `);
        ...
        for (const muro of result.rows) {
            try {
                ...
                // Actualizar en BD
                await db_1.default.query('UPDATE muro SET tipo_brace_seleccionado = $1, factor_w2 = $2 WHERE pid = $3', [tipo_calculado, factor_w2, pid]);
                procesados++;
...
exports.autoRecalcularTiposBraces = autoRecalcularTiposBraces;
/**
 * Actualizar factor W2 en todos los muros
 * POST /api/calculos/actualizar-factor-w2
 */
const actualizarFactorW2 = async (req, res) => {
    try {
        ...
        const result = await db_1.default.query('UPDATE muro SET factor_w2 = $1', [factor_w2_num]);
...
exports.actualizarFactorW2 = actualizarFactorW2;
// Nueva función para actualización masiva de braces
const actualizarBracesMasivo = async (req, res) => {
    try {
