...
export interface RectangularArmadoGrupo {
  numero: string;
  eje: string;
  muros: string;
  largo: string;
  alto: string;
  ancho: string;
  aceroLongTipo: string;
  aceroLongLong: string;
  aceroLongPeso: string;
  aceroTransTipo: string;
  aceroTransLong: string;
  aceroTransPeso: string;
  concretoVol: string;
  concretoPeso: string;
  alambreLong: string;
  alambrePeso: string;
...
export interface RectangularConfigGlobal {
  varillasLong: {
    tipo: string;
  ...
  construccion: {
    resistenciaConcreto: string;
    factorDesperdicio: string;
  };
  alambre: {
    diametro: string;
    longitudVuelta: string;
    factorDesperdicio: string;
  };
}
...
export interface RectangularData {
  ...
  configGlobal?: RectangularConfigGlobal;
...
export interface TriangularArmadoGrupo {
  numero: string;
  eje: string;
  muros: string;
  dimensiones: string;
...
export interface TriangularConfigGlobal {
  geometria: {
    base: string;
    cantVarillasLong: string;
  };
  varillasLong: {
    tipo: string;
  };
...
export interface TriangularData {
  ...
  gruposSecuenciales?: { headers: string[]; rows: string[][] };
  configGrupos?: { headers: string[]; rows: string[][] };
  configGlobal?: TriangularConfigGlobal;
  resumenTotales?: {
    concreto: string;
    acero: string;
    alambre: string;
...
export async function generarInformePDF(
  ...
  rectangularData?: RectangularData,
  triangularData?: TriangularData
): Promise<Buffer> {
...
function drawGenericTablePDF(
  doc: any, headers: string[], rows: string[][], startY: number,
  marginX: number, contentW: number, headerColor: string, title?: string
): number {
  const pageH = doc.page.height;
  let y = startY;
  ...
  if (title) {
    if (y + headerH + rowH * rows.length + 40 > pageH - 40) { doc.addPage(); y = 50; }
    doc.fontSize(10).fillColor('#1f1f1f').font('Helvetica-Bold').text(title, marginX, y);
    y += 16;
  }

  // Check page break
  if (y + headerH + rowH * Math.min(rows.length, 3) > pageH - 40) { doc.addPage(); y = 50; }

  // Header
  doc.save(); doc.rect(marginX, y, contentW, headerH).fill(headerColor); doc.restore();
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (i % 2 === 0) { doc.save(); doc.rect(marginX, y, contentW, rowH).fill('#f5f5f5'); doc.restore(); doc.fillColor('#000000'); }
    for (let c = 0; c < colCount; c++) {
...
function crearPaginasRectangularPDF(doc: any, rectData: RectangularData) {
  const pageW = doc.page.width;
  const marginX = 40;
  const contentW = pageW - marginX * 2;
  ...
  if (rectData.configGlobal) {
    ...
    const drawConfigBlock = (title: string, pairs: [string, string][], color: string) => {
      if (y + 30 + pairs.length * 22 > pageH - 40) { doc.addPage(); y = 50; }
  ...
  if (rectData.armadoResultados && rectData.armadoResultados.grupos.length > 0) {
    ...
    const armadoGroups: { title: string; color: string; headers: string[]; getData: (g: RectangularArmadoGrupo) => string[][] }[] = [
      {
        title: 'DEADMAN',
        color: '#e3f2fd',
    ...
    for (const ag of armadoGroups) {
      ...
      if (y + neededH > pageH - 40) { doc.addPage(); y = 50; }
    ...
    if (tot) {
      if (y + 40 > pageH - 40) { doc.addPage(); y = 50; }
      doc.fontSize(9).fillColor('#1f1f1f').font('Helvetica-Bold')
...
function crearPaginasTriangularPDF(doc: any, triData: TriangularData) {
  const pageW = doc.page.width;
  const pageH = doc.page.height;
  const marginX = 40;
  const contentW = pageW - marginX * 2;
  ...
  if (triData.armadoResultados && triData.armadoResultados.grupos.length > 0) {
    ...
    const armadoGroups: { title: string; color: string; headers: string[]; getData: (g: TriangularArmadoGrupo) => string[][] }[] = [
      {
        title: 'DEADMAN',
        color: '#5ba8c8',
...
export async function generarInformeDOCX(
  ...
  function buildDocxTable(headers: string[], rows: string[][], headerColor: string, altRowColor?: string): Table {
    const colCount = headers.length;
      ...
      const cells = row.map((val, ci) =>
        new TableCell({
          shading: idx % 2 === 0 && altRowColor
            ? { type: ShadingType.CLEAR, fill: altRowColor }
            : undefined,
          children: [
            new Paragraph({
              alignment: ci === 0 ? AlignmentType.LEFT : AlignmentType.CENTER,
  ...
  if (rectangularData) {
    ...
    if (rectangularData.gruposMuertos || rectangularData.gruposSecuenciales) {
      const grpChildren: (Paragraph | Table)[] = [];
      ...
      grpChildren.push(
        new Paragraph({
          ...
          spacing: { after: 200, before: 200 },
          children: [
            new TextRun({ text: 'AGRUPACIÓN DE MUERTOS', bold: true, size: 32, color: '1f1f1f', font: 'Arial' }),
          ],
        })
      );
      grpChildren.push(
        new Paragraph({
          spacing: { after: 200 },
          border: {
            bottom: { style: BorderStyle.SINGLE, size: 4, color: '0d6efd', space: 1 },
          },
          children: [new TextRun({ text: ' ', size: 4 })],
        })
      );
      ...
      if (rectangularData.gruposMuertos) {
        grpChildren.push(
          new Paragraph({
            ...
            children: [
              new TextRun({ text: 'Resumen por Muertos', bold: true, size: 20, color: '0d6efd', font: 'Arial' }),
            ],
          })
        );
        grpChildren.push(buildDocxTable(
          rectangularData.gruposMuertos.headers,
          rectangularData.gruposMuertos.rows,
          '0d6efd',
          'EBF5FB'
        ));
      }
      ...
      if (rectangularData.gruposSecuenciales) {
        grpChildren.push(
          new Paragraph({
            spacing: { before: 200, after: 80 },
            children: [
              new TextRun({ text: 'Grupos Secuenciales', bold: true, size: 20, color: '6f42c1', font: 'Arial' }),
            ],
          })
        );
        grpChildren.push(buildDocxTable(
          rectangularData.gruposSecuenciales.headers,
          rectangularData.gruposSecuenciales.rows,
          '6f42c1',
          'F4ECF7'
        ));
      }
    ...
    if (rectangularData.configGrupos) {
      const cfgChildren: (Paragraph | Table)[] = [];
      ...
      cfgChildren.push(
        new Paragraph({
          ...
          spacing: { after: 200, before: 200 },
          children: [
            new TextRun({ text: 'CONFIGURACIÓN DE GRUPOS', bold: true, size: 32, color: '1f1f1f', font: 'Arial' }),
          ],
        })
      );
      cfgChildren.push(
        new Paragraph({
          spacing: { after: 200 },
          border: {
            bottom: { style: BorderStyle.SINGLE, size: 4, color: '28a745', space: 1 },
          },
          children: [new TextRun({ text: ' ', size: 4 })],
        })
      );
      ...
      if (cfgData.headers.length > 8) {
        ...
        cfgChildren.push(
          new Paragraph({
            spacing: { before: 100, after: 80 },
            children: [
              new TextRun({ text: 'Datos del Grupo', bold: true, size: 20, color: '28a745', font: 'Arial' }),
            ],
          })
        );
        ...
        cfgChildren.push(
          new Paragraph({
            spacing: { before: 200, after: 80 },
            children: [
              new TextRun({ text: 'Dimensiones y Materiales', bold: true, size: 20, color: '28a745', font: 'Arial' }),
            ],
          })
        );
    ...
    if (rectangularData.configGlobal) {
      ...
      const cfgGlobalChildren: (Paragraph | Table)[] = [];

      cfgGlobalChildren.push(
        new Paragraph({
          alignment: AlignmentType.CENTER,
          spacing: { after: 200, before: 200 },
          children: [
            new TextRun({ text: 'CONFIGURACIÓN GLOBAL', bold: true, size: 32, color: '1f1f1f', font: 'Arial' }),
          ],
        })
      );
      cfgGlobalChildren.push(
        new Paragraph({
          spacing: { after: 200 },
          border: {
            bottom: { style: BorderStyle.SINGLE, size: 4, color: '6f42c1', space: 1 },
          },
          children: [new TextRun({ text: ' ', size: 4 })],
        })
      );
      ...
      const addConfigSection = (title: string, pairs: [string, string][], color: string) => {
        cfgGlobalChildren.push(
          new Paragraph({
            spacing: { before: 200, after: 100 },
            children: [
              new TextRun({ text: title, bold: true, size: 22, color, font: 'Arial' }),
            ],
          })
        );
        cfgGlobalChildren.push(buildDocxTable(
          ['Parámetro', 'Valor'],
    ...
    if (rectangularData.armadoResultados && rectangularData.armadoResultados.grupos.length > 0) {
      const armChildren: (Paragraph | Table)[] = [];
      ...
      armChildren.push(
        new Paragraph({
          ...
          spacing: { after: 200, before: 200 },
          children: [
            new TextRun({ text: 'ARMADO RECTANGULAR - RESULTADOS', bold: true, size: 32, color: '1f1f1f', font: 'Arial' }),
          ],
        })
      );
      armChildren.push(
        new Paragraph({
          spacing: { after: 200 },
          border: {
            bottom: { style: BorderStyle.SINGLE, size: 4, color: 'dc3545', space: 1 },
          },
          children: [new TextRun({ text: ' ', size: 4 })],
        })
      );
      ...
      const armadoGroups: { title: string; headerColor: string; altColor: string; headers: string[]; getData: (g: RectangularArmadoGrupo) => string[][] }[] = [
        {
          title: 'DEADMAN',
          headerColor: '2196F3',
          altColor: 'E3F2FD',
      ...
      for (const ag of armadoGroups) {
        ...
        armChildren.push(
          new Paragraph({
            spacing: { before: 200, after: 80 },
            children: [
              new TextRun({ text: ag.title, bold: true, size: 22, color: ag.headerColor, font: 'Arial' }),
            ],
          })
        );
      ...
      if (tot) {
        armChildren.push(
          new Paragraph({
            ...
            children: [
              new TextRun({ text: 'TOTALES PROYECTO:', bold: true, size: 20, color: '1f1f1f', font: 'Arial' }),
            ],
          })
        );
        const totItems = [
          ...
          armChildren.push(
            new Paragraph({
              ...
              children: [
                ...
                new TextRun({ text: item, size: 18, color: '333333', font: 'Arial' }),
              ],
            })
          );
        }
    ...
    if (triangularData.gruposMuertos || triangularData.gruposSecuenciales) {
      ...
      grpChildren.push(
        new Paragraph({
          spacing: { after: 200 },
          border: {
            bottom: { style: BorderStyle.SINGLE, size: 4, color: 'fd7e14', space: 1 },
          },
          children: [new TextRun({ text: ' ', size: 4 })],
        })
      );
      ...
      if (triangularData.gruposMuertos) {
        grpChildren.push(
          new Paragraph({
            spacing: { before: 200, after: 100 },
            children: [
              new TextRun({ text: 'Resumen por Muertos', bold: true, size: 22, color: '0d6efd', font: 'Arial' }),
            ],
          })
        );
      ...
      if (triangularData.gruposSecuenciales) {
        grpChildren.push(
          new Paragraph({
            spacing: { before: 300, after: 100 },
            children: [
              new TextRun({ text: 'Grupos Secuenciales', bold: true, size: 22, color: '6f42c1', font: 'Arial' }),
            ],
          })
        );
    ...
    if (triangularData.configGrupos) {
      ...
      if (cfg.headers.length > 8) {
        ...
        cfgChildren.push(
          new Paragraph({
            spacing: { before: 100, after: 80 },
            children: [new TextRun({ text: 'Datos del Grupo', bold: true, size: 20, color: '28a745', font: 'Arial' })],
          })
        );
        cfgChildren.push(buildDocxTable(headers1, rows1, '28a745', 'EAFAF1'));
        cfgChildren.push(
          new Paragraph({
            spacing: { before: 200, after: 80 },
            children: [new TextRun({ text: 'Dimensiones y Materiales', bold: true, size: 20, color: '28a745', font: 'Arial' })],
          })
        );
        cfgChildren.push(buildDocxTable(headers2, rows2, '28a745', 'EAFAF1'));
      } else {
    ...
    if (triangularData.configGlobal) {
      ...
      cfgGlobalChildren.push(
        new Paragraph({
          ...
          spacing: { after: 200, before: 200 },
          children: [
            new TextRun({ text: 'CONFIGURACIÓN GLOBAL - TRIANGULAR', bold: true, size: 32, color: '1f1f1f', font: 'Arial' }),
          ],
        })
      );
      cfgGlobalChildren.push(
    ...
    if (triangularData.armadoResultados && triangularData.armadoResultados.grupos.length > 0) {
      ...
      armChildren.push(
        new Paragraph({
          ...
          spacing: { after: 200, before: 200 },
          children: [
            new TextRun({ text: 'ARMADO TRIANGULAR - RESULTADOS', bold: true, size: 32, color: '1f1f1f', font: 'Arial' }),
          ],
        })
      );
      armChildren.push(
      ...
      const armadoGroups: { title: string; headerColor: string; altColor: string; headers: string[]; getData: (g: TriangularArmadoGrupo) => string[][] }[] = [
        {
          title: 'DEADMAN',
          headerColor: '2196F3',
          altColor: 'E3F2FD',
