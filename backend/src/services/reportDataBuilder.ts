...
export function normalizePaneles(paneles: (PanelCalculado | PanelCalculadoPaneles | any)[]): PanelCalculado[] {
  return paneles.map(panel => {
    ...
    const result = panel as PanelCalculado;
...
function normalizeTablaMuertos(raw?: MuertoResumen[]): MuertoResumen[] | undefined {
